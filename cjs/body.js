...
var import_fetch_blob = __toESM(require("fetch-blob"), 1);
...
const pipeline = (0, import_util.promisify)(import_stream.default.pipeline);
...
class Body {
  constructor(body, {
    ...
    if (body === null) {
      ...
      body = import_buffer.Buffer.from(body.toString());
    } else if ((0, import_is.isBlob)(body)) {
    } else if (import_buffer.Buffer.isBuffer(body)) {
    } else if (import_util.types.isAnyArrayBuffer(body)) {
      body = import_buffer.Buffer.from(body);
    } else if (ArrayBuffer.isView(body)) {
      body = import_buffer.Buffer.from(body.buffer, body.byteOffset, body.byteLength);
    } else if (body instanceof import_stream.default) {
    } else if (body instanceof import_esm_min.FormData) {
      body = (0, import_esm_min.formDataToBlob)(body);
      boundary = body.type.split("=")[1];
    } else {
      body = import_buffer.Buffer.from(String(body));
    }
    let stream = body;
    if (import_buffer.Buffer.isBuffer(body)) {
      stream = import_stream.default.Readable.from(body);
    } else if ((0, import_is.isBlob)(body)) {
      stream = import_stream.default.Readable.from(body.stream());
    }
    this[INTERNALS] = {
      body,
      stream,
